package com.startaideia.vuttr.configuration.swagger;

import java.util.Collections;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import springfox.documentation.builders.PathSelectors;
import springfox.documentation.builders.RequestHandlerSelectors;
import springfox.documentation.service.ApiInfo;
import springfox.documentation.service.Contact;
import springfox.documentation.spi.DocumentationType;
import springfox.documentation.spring.web.plugins.Docket;
import springfox.documentation.swagger2.annotations.EnableSwagger2;

/**
 * 
 * Classe responsável pela configuração do Swagger para geração da documentação da API
 */
@Configuration
@EnableSwagger2
public class SpringFoxConfig {

	/*
	 * Método de configuração das atribuições em relação a geraração da documentação da API
	 */
	@Bean
    public Docket api() { 
        return new Docket(DocumentationType.SWAGGER_2)  
          .select()                                  
          .apis(RequestHandlerSelectors.basePackage("com.startaideia.vuttr.api.controller"))              
          .paths(PathSelectors.any())                          
          .build()
          .apiInfo(apiInfo());
          
    }
	
	/*
	 * Método de configuração das informações referentes a API 
	 */
	private ApiInfo apiInfo() {
	    return new ApiInfo(
	      "REST API VUTTR (Very Useful Tools to Remember)", 
	      "Tool management application.", 
	      "API TOOLS", 
	      "Terms of service", 
	      new Contact("Robson Silva", "https://www.linkedin.com/in/robson-silva-0b79291b6/", "robson_r.silva@hotmail.com.br"), 
	      "License of API", "https://swagger.io/", Collections.emptyList());
	}
}
